...
//				m_tilePredictor.takeOffTile(randNum);
...
//	m_tilePredictor.takeOffTile(m_hintTile);
}
...
BitBoard BitBoard::copyCurrentBoard()
{
	...
	b_new.m_tilePredictor = m_tilePredictor;
...
void BitBoard::move(MoveDirection Direction)
{
	...
	for(int i=0;i<16;i+=4)
	{
		...
		if(static_cast<unsigned long long>(FixedValue::afterMovingTable[rowValue])!=rowValue)	//have a slot
		{
...
int BitBoard::getMaxValueInBoard()
{
	...
	{
		tileValue = static_cast<unsigned long long>((m_board&mask)>>shiftNum);
		if(max < tileValue)
			max = tileValue;
	}
	return max;		// 0~14
...
int BitBoard::get_all_possible_board(BitBoard boards[], float boardsPossibility[], bool has_hint)
{
	...
	if(has_hint)
	{
		int hintTile = getHintTile();	//if hint == 4 , means '+' with (set in main.cpp)
		for(int i=0;i<4;i++)
		{
			if(!hasSlot[i])
				continue;
			...
			if(hintTile<=3)
			{
				...
				boardsPossibility[possible_board_size] = (1.0) / slotCount;
				if(hintTile==3)
					boards[possible_board_size].setScore(m_gameScore + power3(hintTile-2));
				possible_board_size++;
			}	
			else
			for(int j=4;j<=maxTileCount;j++)
			{
				float appearProbability = 1.0/(maxTileCount-3);
				boards[possible_board_size] = copyCurrentBoard();
				addInTileMask = static_cast<unsigned long long>(j)<<shiftTable[i];
				boards[possible_board_size].m_board |= addInTileMask;
				boardsPossibility[possible_board_size] = appearProbability / slotCount;
				boards[possible_board_size].setScore(m_gameScore + power3(hintTile-2));
				possible_board_size++;
			}			
		}
	...
	else
	{
		for(int i=0;i<4;i++)
		{
			...
			{
				float appearProbability = m_tilePredictor.getProbability(j,m_maxValueInBoard);
				if(appearProbability <= 0.0001)
					continue;
